...
/* Pedido.obtenerPedidosPorUsuario = async (ususario) => {
    const result = await sequelize.query(
        'SELECT e.descripcion Estado, o.fecha_creacion Hora, op.id_orden Numero, p.nombre Producto, p.descripcion Producto, f.descripcion FormaPago, op.total Valor, u.usuario Usuario, u.direccion FROM productos p, formas_pago f, estados e, usuarios u, ordenes o, ordenes_productos op WHERE u.id = o.id_usuario AND op.id_orden = o.id AND op.id_producto = p.id AND f.id = o.id_forma_pago AND e.id = o.id_estado AND u.usuario = ?',{
            replacements: [ususario], type: sequelize.QueryTypes.SELECT
        }
    );
    return result;
} */
